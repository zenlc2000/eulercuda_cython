...
void phase2HostLaunch(KEY_PTR   bufferK,VALUE_PTR bufferV,
	...
	cutilSafeCall( cudaMemcpy(h_bufferK, bufferK, length * (KEY_SIZE), cudaMemcpyDeviceToHost));
	cutilSafeCall( cudaMemcpy(h_bufferV, bufferV, length * (VALUE_SIZE), cudaMemcpyDeviceToHost));
	cutilSafeCall( cudaMemcpy(h_start, start,sizeof(unsigned int)* bucketCount, cudaMemcpyDeviceToHost));
	cutilSafeCall( cudaMemcpy(h_count, count, sizeof(unsigned int)* bucketCount, cudaMemcpyDeviceToHost));
	...
	cutilSafeCall( cudaMemcpy( bucketSeed, h_bucketSeed, sizeof(unsigned int)* bucketCount,cudaMemcpyHostToDevice) );
	cutilSafeCall( cudaMemcpy( TK, h_TK, KEY_SIZE*BLOCK_SIZE*bucketCount,cudaMemcpyHostToDevice) );
	cutilSafeCall( cudaMemcpy( TV, h_TV, VALUE_SIZE*BLOCK_SIZE*bucketCount,cudaMemcpyHostToDevice) );
...
void createHashTable2(KEY_PTR d_keys,VALUE_PTR d_values, unsigned int length, KEY_PTR *  d_TK,VALUE_PTR * d_TV,unsigned int * tableLength, unsigned int ** d_bucketSeed,unsigned int * bucketCount){
	...
	*bucketCount=(length /409)+1; //ceil
	...
	//allocate count 
	cutilSafeCall( cudaMalloc( (void**) &d_count, bucketDataSize));
	
	
	//initialize offset to zero
	cutilSafeCall( cudaMemset(d_offset,0,dataSize));
	//initialize count to zero
	cutilSafeCall( cudaMemset(d_count,0,bucketDataSize));
	...
	/***************     Cuckoo Hashing        ******************/
	cutilSafeCall( cudaMalloc( (void**) d_bucketSeed, bucketDataSize));
	...
	cutilSafeCall( cudaMalloc( (void**) &d_randomSeed,randomSeedSize));
	for(unsigned int i=0;i<*bucketCount;i++){
		for(int j=0;j<MAX_SEED_COUNT;j++){
			*(h_randomSeed+i*MAX_SEED_COUNT+j)=rand();
		}
	}
	cutilSafeCall( cudaMemcpy( d_randomSeed, h_randomSeed, randomSeedSize,cudaMemcpyHostToDevice) );
	free(h_randomSeed);
	...
	cutilSafeCall(cudaFree(d_randomSeed));
